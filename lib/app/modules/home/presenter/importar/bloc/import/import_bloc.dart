...
class ImportBloc extends Cubit<ImportState> {
  ...
  deletePLanillaByAnioMes(int anio, int mes) async {
    final res = await DBProvider.db.deletePlanillaByAnioMes(anio, mes);
//    print(res.toString());
//    emit(DeletedImportState());
  }

  deletePLanillaByAnioMesState(int anio, int mes) async {
    emit(LoadingImportState());
    final res = await DBProvider.db.deletePlanillaByAnioMes(anio, mes);
    if (res > 0) {
      emit(DeletedImportState());
    } else {
      emit(ErrorImportState(message: 'Error al eliminar', nombres: ''));
    }
  }
}
